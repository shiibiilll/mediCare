...
const AddService = ({ serviceId }) => {
  ...
  const [hasExistingImage, setHasExistingImage] = useState(false); // Upadate the image if found
  ...
  // Compute The Days For Selected Month/Year
  ...
  // Fetch Services When Editing State
  useEffect(() => {
    let mounted = true;
  ...
  }, [serviceId, API_BASE]); // Pre fetch for particular service if present
  ...
  function handleImageChange(e) {
    ...
    // Remove existing image when choose new file
    setRemoveImage(false);
    setHasExistingImage(false);
  }
  ...
  function resetForm() {
    if (imagePreview && imagePreview.startsWith("blob:")) {
      try {
        ...
        console.error("Error when revoking image:", err);
      }
    }
    setImagePreview(null);
    setImageFile(null);
  ...
  // Convert 12Hrs into date object
  ...
  // Prevent user for selecting the past time for the particular date
    ...
    <div className={addServiceStyles.container.main}>
      <div className={addServiceStyles.toast.container}>
        {toast && (
          ...
          >
            ...
            >
              <XCircle className="w-4 h-4 text-gray-400 hover:text-grey-600" />
            </button>
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className={addServiceStyles.container.form}>
        <div
         ...
         justify-between sm:items-center mb-6 sm:mb-8 gap-4"
        >
          <div>
            ...
            >
              ...
              <div className="w-full flex gap-2 items-center">
                ...
                <button
                  ...
                  className={`${addServiceStyles.buttons.uploadImage} cursor-pointer`}
                >
                ...
                {(imagePreview || hasExistingImage) && (
                  <button
                    ...
                    className={`${addServiceStyles.buttons.removeImage} cursor-pointer`}
                  >
