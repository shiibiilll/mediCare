...
// Remove duplicate slot and return the slot filter by time
...
export async function createDoctor(req, res) {
  try {
    ...
    // Checking the email is already exist
...
export async function toggleAvailability(req, res) {
  try {
    ...
    if (typeof doc.availability === "boolean") {
      ...
      await doc.save();
      const out = normalizeDocForClient(doc.Object());
      delete out.password;
      return res.json({ success: true, data: out });
    }
  } catch (err) {
