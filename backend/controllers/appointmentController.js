...
export async function createAppointment(req, res) {
  try {
    ...
    if (!doctorId || !patientName || !mobile || !time) {
...
//Confirm online payment and make it paid
export async function confirmPayment(req, res) {
  try {
    ...
    try {
      ...
      console.error("Stripe retrive session error:", error);
      return res.status(404).json({
        success: false,
...
export async function getAppointmentsByDoctor(req, res) {
  try {
    ...
    console.error("Get appointment by doctor error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}

//Register user count
export async function registerUserCount(req, res) {
  try {
    const totalUser = await clerkClient.users.getCount();
    return res.json({ success: true, totalUser });
  } catch (error) {
    console.error("Register user count error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
