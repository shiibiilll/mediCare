...
//Parse time and return AM or PM
...
export async function confirmServicePayment(req, res) {
  try {
    ...
    console.error("Create service payment error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export async function getServiceAppointmentById(req, res) {
  try {
    ...
    console.error("Get service appointment by id error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export async function updateServiceAppointment(req, res) {
  try {
    ...
    //Check if it is filled, if yes update the field
...
export async function getServiceAppointmentStats(req, res) {
  try {
    ...
    ]); //This will give total appointment, complete appointment
...
//Get appointment for the patient
export async function getServiceAppointmentsByPatient(req, res) {
  try {
    ...
    console.error("Get service appointment by patient error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
